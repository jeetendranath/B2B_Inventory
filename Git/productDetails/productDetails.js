...
import getStockBySKU from '@salesforce/apex/B2BInventory_Class.getStockBySKU';
import { CurrentPageReference } from 'lightning/navigation';
import updateReservedBySKU from '@salesforce/apex/B2BInventory_Class.updateReservedBySKU';
import createCaseNew from '@salesforce/apex/B2BInventory_Class.createCaseNew';
...
export default class ProductDetails extends LightningElement {
    ...
    get displayableProduct() {
        return {
            ...
            //availableqty: this.availableStock.data,
    ...
    updateReserve(event){
        updateReservedBySKU({productId:this.recordId,
            reserveQty:event.detail.quantity})
        .then((result)=>
        {
        this.availableqty =result.Available_for_Purchase__c;
        console.log(result);
    ...
    createCases(){
        ...
        this.dispatchEvent(
            new ShowToastEvent({
                title: 'Case Created :)',
                message: 'We Will notify you when product is available',
                variant: 'success',
                mode: 'dismissable'
            })
        );
    ...
    @track ProductId1 = toString(this.currentUrl);
   
   // availableqty= this.recordId;
   prodid=this.recordId;
   
   

 availableQuantity(){
   getStockBySKU({productId:this.recordId})
    .then((result)=>
    {
        
        this.availableqty =result.qty;
        if(this.availableqty <5)
        {
