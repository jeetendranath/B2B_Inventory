...
import updateReservedCart from '@salesforce/apex/B2BInventory_Class.updateReservedCart';
import cartItemDelete from '@salesforce/apex/B2BInventory_Class.cartItemDelete';
import clearCartHandle from '@salesforce/apex/B2BInventory_Class.clearCartHandle';
...
export default class CartContents extends NavigationMixin(LightningElement) {
    ...
    handleQuantityChanged(evt) {
        ...
        updateCartItem({
            ...
            .then((cartItem) => {
                this.updatedQTY=Number(cartItem.quantity);
                this.Id = cartItem.cartItemId;
                console.log('Updated Quantity '+this.updatedQTY);   /* --------------------------------- */
    ...
    handleCartItemDelete(evt) {
        ...
        console.log('Deleted Id '+ this.deleteCartItemId);
        this.delCartItem(); /*-------------------------------*/
    ...
    handleClearCartButtonClicked() {
        // Step 0: Updates the reserve Quantity
    ...
    updateCartItemInformation(cartItem) {
        ...
        const updatedCartItems = (this.cartItems || []).map((item) => {
            ...
            if (updatedItem.cartItem.cartItemId === cartItem.cartItemId) {
                this.oldQTY=Number(updatedItem.cartItem.quantity);
                
                console.log('Old Quantity '+this.oldQTY);   /* --------------------------------- */
    ...
    updateReservedCart({oldQTY : this.oldQTY , newQTY:this.updatedQTY,cartItemId:this.Id})
    ...
    delCartItem(){
        cartItemDelete({cartItemId:this.deleteCartItemId})
        .then((result)=>
        {
            console.log(result);
        })
    ...
    clearCartHandle(){
        clearCartHandle({cartId:this.recordId})
        .then((result)=>
        {
            console.log(result);
        })
