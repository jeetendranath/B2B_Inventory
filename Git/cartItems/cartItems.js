...
import isQuantityAvailable from '@salesforce/apex/B2BInventory_Class.isQuantityAvailable';
...
export default class Items extends NavigationMixin(LightningElement) {
    ...
    handleQuantitySelectorBlur(blurEvent) {
        ...
        this.quantity1 = blurEvent.target.value;
        this.cartItemId1 = blurEvent.target.dataset.itemId;
       ...
       if(this.quantity1 <= 0)
        ...
        console.log('Cart Item Id '+this.cartItemId1);
        console.log('Present Quantity '+this.quantity1);
        console.log('Available Quantity :' + this.availableqty);
        console.log('cart Item ID '+this.cartItemId1);
        isQuantityAvailable({cartItemid : this.cartItemId1 , QTY : this.quantity1})
        .then((result)=>
        {
            if(result == true)
            {
                const evt = new ShowToastEvent({
                    title: 'Error :(',
                    message: 'Quantity should be not  Greater than Available Quantity',
                    variant: 'error',
                    mode: 'dismissable'
                });
                this.dispatchEvent(evt);
